...
class AdminGroupsControllerCore extends AdminController
{
    ...
    public function renderList()
    {
        ...
        $unidentified_group_information = $this->trans('%group_name% - All persons without a customer account or customers that are not logged in.', ['_raw' => true, '%group_name%' => '<b>' . htmlentities($unidentified->name[$this->context->language->id]) . '</b>'], 'Admin.Shopparameters.Help');
        $guest_group_information = $this->trans('%group_name% - All persons who placed an order through Guest Checkout.', ['_raw' => true, '%group_name%' => '<b>' . htmlentities($guest->name[$this->context->language->id]) . '</b>'], 'Admin.Shopparameters.Help');
        $default_group_information = $this->trans('%group_name% - All persons who created an account on this site.', ['_raw' => true, '%group_name%' => '<b>' . htmlentities($default->name[$this->context->language->id]) . '</b>'], 'Admin.Shopparameters.Help');
