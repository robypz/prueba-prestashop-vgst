...
class SetupGuideDataPresenter
{
    ...
    public function getSetupGuideData(): array
    {
        ...
        if (!$this->isHosted()) {
            array_push($response, $this->buildStep(
                'eBilling',
                'onb.homepage.setupGuide.eBilling.title',
                [
                    [
                        'description' => 'onb.homepage.setupGuide.eBilling.description',
                        'buttons' => [
                            [
                                'cta' => 'onb.homepage.setupGuide.eBilling.button.cta',
                                'href' => 'onb.homepage.setupGuide.eBilling.button.href',
                                'userflow_id' => '',
                            ],
                        ],
                    ],
    ...
    private function isHosted(): bool
    {
        try {
            return \Module::isInstalled('smb_edition')
                && \Module::isEnabled('smb_edition')
                && $this->get('router')->generate('smb_edition_manage_domain_name')
                && is_string($this->getShopID())
                && strlen($this->getShopID()) > 0
            ;
        } catch (\Throwable $e) {
            return false;
        }
